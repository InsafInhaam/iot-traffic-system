...
def draw_polygons(img):
    ...
    if len(current_polygon) > 1:
        pts = (np.array(current_polygon) * scale).astype(np.int32).reshape((-1, 1, 2))
...
def detect_vehicles_yolo(image, lanes):
    ...
    # Simulated "Auto-detecting lanes..."
    display = annotated.copy()
    cv2.putText(display, "Auto-detecting lanes...", (30, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
    cv2.imshow("YOLO Detection", resize_for_display(display))
    cv2.waitKey(1000)  # pause for visual effect

    # Draw polygons (auto-loaded lanes)
    for name, polygon in lanes.items():
...
if __name__ == "__main__":
    ...
    if mode == "c":
        ...
        cv2.imshow("Final Output - Auto Lane Detection", resize_for_display(annotated))
        cv2.waitKey(0)
        cv2.destroyAllWindows()
# ---------------- End ----------------
