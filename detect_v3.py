...
CONF_THRES = 0.5
API_URL = os.getenv(
    "API_URL", "http://127.0.0.1:8000/vehicles/vehicles")  # FastAPI POST
...
emergency_keywords = {"ambulance", "fire truck",
                      "police car"}  # model may not have these labels
...
def grayworld_white_balance(img):
    ...
    scale_b, scale_g, scale_r = avg_gray / \
        (avg_b+1e-6), avg_gray/(avg_g+1e-6), avg_gray/(avg_r+1e-6)
    result[:, :, 0] *= scale_b
    result[:, :, 1] *= scale_g
    result[:, :, 2] *= scale_r
...
def main():
    ...
    while True:
        ...
        if SHOW_PREPROC and pre is not None:
            combo = np.hstack([cv2.resize(pre, (width//2, height)),
